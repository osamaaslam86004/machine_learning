...
def plot_confusion_matrix(y_true, y_pred, classes):
    ...
    plt.savefig("sg_multi_class__OvR_confusion_matrix.png")  # Saves the figure
    plt.show()


def evaluate_sg_classifier_one_vs_rest_strategy(X_train, y_train, X_test, y_test):
    """Evaluates a classifier and prints relevant metrics."""

    # Instantiate SGDClassifier
    sgd_clf = SGDClassifier(random_state=42)
    sgd_clf = OneVsRestClassifier(sgd_clf)  # Apply OvR strategy
