...
def automatic_feature_selection_pipeline(pipeline, X_train, y_train):
    ...
    param_distributions = {
        "preparation__num__imputer__strategy": ["median", "mean"],  # 2 options
        "preparation__num__attribs_adder__add_bedrooms_per_room": [
            True,
            False,
        ],  # 2 options
        "regressor__n_estimators": [50, 100, 150],  # 3 options
    }
    ...
    rnd_search = RandomizedSearchCV(
        ...
        verbose=2,
        n_iter=12,  # every 3 log lines with the same parameters = 1 iteration.
        random_state=42,
    )
    ...
    print("Best Parameters for Training Set:", rnd_search.best_params_)
    ...
    return best_model, rnd_search, X_train, y_train
